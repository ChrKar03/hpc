...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
	...
	for (row = 1; row < inner_limit; row++) {
		for (col = 1; col <= inner_limit - 4; col += 4) {
			...
			diff = output[row * SIZE + col] - golden[row * SIZE + col];
			PSNR += diff * diff;
			...
			diff = output[row * SIZE + col + 1] - golden[row * SIZE + col + 1];
			PSNR += diff * diff;
			...
			diff = output[row * SIZE + col + 2] - golden[row * SIZE + col + 2];
			PSNR += diff * diff;
			...
			diff = output[row * SIZE + col + 3] - golden[row * SIZE + col + 3];
			PSNR += diff * diff;
		}
