...
#define UNROLLED_1(off) \
	p = pow(convolution2D(i, j + off, input, horiz_operator), 2) + \
		pow(convolution2D(i, j + off, input, vert_operator), 2); \
	res = (int)sqrt(p); \
	output[i * SIZE + j + off] = (res > 255) ? 255 : (unsigned char)res;

#define UNROLLED_2(off) \
	t = pow((output[i * SIZE + j + off] - golden[i * SIZE + j + off]), 2); \
	PSNR += t;
...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
	...
	for (i = 1; i < SIZE - 1; i++) {
		...
		/* Handle remaining pixels if SIZE-2 is not divisible by 4 */
		for (; j < SIZE - 1; j++) {
			UNROLLED_1(0);
		}
	}
	...
	for (i = 0; i < SIZE; i++) {
		for (j = 0; j < SIZE - UNROLL_FACTOR + 1; j += UNROLL_FACTOR) {
			UNROLLED_2(0);
			UNROLLED_2(1);
			UNROLLED_2(2);
			UNROLLED_2(3);
		...
		/* Handle remaining pixels if SIZE is not divisible by 4 */
		for (; j < SIZE; j++) {
			UNROLLED_2(0);
		}
	}
