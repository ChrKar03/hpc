...
#define INLINE_CONVOLUTION2D(off, operator) ( \
	input[(i - 1) * SIZE + j + off - 1] * operator[0][0] + \
	input[(i - 1) * SIZE + j + off    ] * operator[0][1] + \
	input[(i - 1) * SIZE + j + off + 1] * operator[0][2] + \
	input[(i    ) * SIZE + j + off - 1] * operator[1][0] + \
	input[(i    ) * SIZE + j + off    ] * operator[1][1] + \
	input[(i    ) * SIZE + j + off + 1] * operator[1][2] + \
	input[(i + 1) * SIZE + j + off - 1] * operator[2][0] + \
	input[(i + 1) * SIZE + j + off    ] * operator[2][1] + \
	input[(i + 1) * SIZE + j + off + 1] * operator[2][2] \
);

#define UNROLL_FACTOR 16

#define UNROLLED(off) \
	ch = INLINE_CONVOLUTION2D(off, horiz_operator); \
	cv = INLINE_CONVOLUTION2D(off, vert_operator); \
	p = pow(ch, 2) + pow(cv, 2); \
	res = (int)sqrt(p); \
	idx = i * SIZE + j + off; \
