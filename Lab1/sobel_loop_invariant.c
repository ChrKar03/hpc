...
#define INLINE_CONVOLUTION2D(operator) ( \
	input[upper  - 1] * operator[0][0] + \
	input[upper     ] * operator[0][1] + \
	input[upper  + 1] * operator[0][2] + \
	input[middle - 1] * operator[1][0] + \
	input[middle    ] * operator[1][1] + \
	input[middle + 1] * operator[1][2] + \
	input[lower  - 1] * operator[2][0] + \
	input[lower     ] * operator[2][1] + \
	input[lower  + 1] * operator[2][2]  \
);
...
#define UNROLLED(off) \
	...
	ch = INLINE_CONVOLUTION2D(horiz_operator); \
	cv = INLINE_CONVOLUTION2D(vert_operator); \
	p = pow(ch, 2) + pow(cv, 2); \
	res = (int)sqrt(p); \
	idx = row_base + j + off; \
	output[idx] = (res > 255) ? 255 : (unsigned char)res; \
	t = pow((output[idx] - golden[idx]), 2); \
	PSNR += t;
...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
	...
	int ch, cv; // convolution horizontal and vertical
	int idx;    // index in output and golden arrays
	int row_base, upper_row_base, lower_row_base; // base indices for rows
	int upper, middle, lower; // indices for convolution
