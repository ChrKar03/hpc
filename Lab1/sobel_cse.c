...
#define UNROLLED(off) \
	...
	diff = output[middle] - golden[middle]; \
	t = pow(diff, 2); \
	PSNR += t;
...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
	...
	int u0, u1, u2, m0, m1, m2, l0, l1, l2; // pixels for inlined convolution
	int temp0, temp1, temp2, temp3, temp4, temp5, temp6, temp7, temp8; // temps for inlined convolution
	int p1, p2; // squared convolutions
	double diff;    // difference between computed and golden pixel
